...
class IndustryAnalyzer:
    ...
    async def generate_queries(self, company_name: str) -> List[str]:
        return [
            f"{company_name} industry market size growth projections",
            f"{company_name} sector trends regulation competition",
            f"{company_name} major players market share analysis",
            f"{company_name} industry outlook competitive landscape",
        ]
    ...
    async def search(self, queries: List[str]) -> List[Dict]:
        ...
        print(f"IndustryAnalyzer: {len(all_results)} unique docs")
        return all_results

    async def run(self, state: ResearchState) -> dict:
        company = state["company_name"]
        print(f"IndustryAnalyzer starting for: {company}")
